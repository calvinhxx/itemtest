...
工程 MUST 在 `src/common/QtCompat.h` 提供单一头文件，集中封装 Qt5/Qt6 之间的事件类型差异：
...
- **THEN** 命中 MUST 仅限于 `src/common/QtCompat.h` 内部，以及测试中针对版本断言的少量必要语句
...
`src/view/textfields/TextBlock.h` MUST NOT 包含 `using Label = TextBlock;` 之类的别名。Qt5 / Windows SDK 在某些头文件中已存在 `Label` 符号（如 X11 兼容头、MFC/ATL），引入该别名会触发 MSVC `C2371: Label 重定义`。

如调用方需要简称，MUST 在调用处局部 `using` 而非污染命名空间。

#### Scenario: TextBlock.h 在 Qt5 / MSVC 编译通过
- **WHEN** 在 Qt 5.15 + MSVC 环境编译包含 `TextBlock.h` 的翻译单元
