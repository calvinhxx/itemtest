# PasswordBox Specification

## Purpose

规范 textfields 模块中的 Fluent `PasswordBox` 控件：它继承自 `LineEdit`，提供密码输入专用的 `password` 属性、`passwordChanged` 信号、`PasswordRevealMode` 显示策略、可选 header、右侧 reveal 按钮、WinUI 3 风格视觉状态，以及对应的主题与测试要求。
