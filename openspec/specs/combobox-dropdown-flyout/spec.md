# combobox-dropdown-flyout Specification

## Purpose

`ComboBox` dropdowns are anchored transient list surfaces. They use `Flyout` semantics instead of dialog semantics so they can light-dismiss, remain attached to the owning top-level widget, and preserve ComboBox-specific selection behavior without exposing dialog result APIs.
